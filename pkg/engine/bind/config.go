...
// RenderConfig renders an EngineConfig to a BIND named.conf string.
func RenderConfig(config engine.EngineConfig) (string, error) {
	normalized := normalizeConfig(config)
	if err := validateBindCompatibility(normalized); err != nil {
		return "", err
	}
	...
	tmpl, err := template.New("bind").Parse(engine.BindConfigTemplate)
	if err != nil {
		return "", err
	}
...
func validateBindCompatibility(config engine.EngineConfig) error {
	for i, upstream := range config.Upstreams {
		if upstream.Transport != engine.UpstreamTransportDNS {
			return fmt.Errorf("upstreams[%d].transport %q is not supported by bind engine", i, upstream.Transport)
		}
	}
